...
export const ShopDetails = ({ route, navigation }: any) => {
  ...
  return (
    <ShopDetailsContainer>
      <Backgroundimage source={require('../../../assets/images/mask.png')} resizeMode={FastImage.resizeMode.cover} />
      ...
      <MainSheet scroll sheetHeight={0.4}>
        <ItemName>{routes.service_name}</ItemName>
        <DescriptionArea description={routes.description} />
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <View>
            <SizeArea size={routes.size} />
          </View>
          <View>
        ...
        <View style={{ flexDirection: 'row' }}>
          <View >
            <ColorContianer color={routes.color} />
          </View>
      ...
      <ReserveNowArea>
        <Text style={{ color: 'white', fontSize: 20, fontWeight: 'bold' }}>JD {routes.price}</Text>
