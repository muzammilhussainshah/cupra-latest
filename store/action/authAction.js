...
export const _facebookAuth = (navigation, getSocialId, getSocialtype) => {
  return async dispatch => {
    ...
    try {
      ...
      let signInMethod = auth()
        ...
        .then(async resp => {
          ...
          if (response.data.status === 200) {
            dispatch({type: CURRENTUSER, payload: response.data.data.data});
            try {
              await AsyncStorage.setItem('socialId', socialId);
              await AsyncStorage.setItem('socialType', socialType);
              dispatch(_loading(false));
          ...
          } else if (response.data.error.messageEn == 'Invalid Credentials') {
            dispatch(_loading(false));
            {
              navigation &&
                navigation.navigate('resetPassword', {
      ...
      dispatch(_loading(false));
      //   dispatch(_error(err.data.error.messageEn));
      console.log(
        err.response,
        'error from _resetNewPassword',
        JSON.parse(JSON.stringify(err.message)),
