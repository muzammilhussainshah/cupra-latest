...
export const getMonth = (n: number) => {
  const date = moment(String(n), 'M').format('MMM YYYY');
...
export const toLocaleDateString = (d: Date) => {
  ...
  return `${date.getDate() < 10 ? `0${date.getDate()}` : date.getDate()}/${date.getMonth() < 10 ? `0${date.getMonth() + 1}` : date.getMonth() + 1
    }/${date.getFullYear()}`;
};
